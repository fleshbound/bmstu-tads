...
int array_queue_expand(array_queue_t *array_queue)
{
    ...
    expanded_data = realloc(array_queue->data, array_queue->max_size * sizeof(int) * EXPAND_FACTOR);
    ...
    array_queue->max_size = array_queue->size * EXPAND_FACTOR;
...
int array_queue_insert(array_queue_t *array_queue, int value, int index)
{
    ...
    index = MIN(index, (int)array_queue->size);
