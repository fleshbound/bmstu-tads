...
void show_err(const int err_code)
{
    ...
    if (err_code == ERR_FULL_HASH) fprintf(stderr, MSG_ERR_FULL_HASH);
    if (err_code == ERR_FILE) fprintf(stderr, MSG_ERR_FILE);
    if (err_code == ERR_EMPTY_HASH) fprintf(stderr, MSG_ERR_EMPTY_HASH);
    if (err_code == ERR_EXISTING_KEY) fprintf(stderr, MSG_ERR_EXISTING_KEY);
}
