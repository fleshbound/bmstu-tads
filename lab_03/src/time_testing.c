...
void time_testing(int n, int m, int fill)
{
    ...
    time_b = tick() / (CLOCKS_PER_SEC / 1000000LLU);

    for (int i = 0; i < ITER; i++)
        add_matrices(matr1, matr2, matr3, n, m);

    time_e = tick() / (CLOCKS_PER_SEC / 1000000LLU);

    printf("%15llu (%12zu)|", (time_e - time_b) / ITER, sizeof(int)*n*m*3);
    ...
    for (int i = 0; i < ITER; i++)
        add_matrices_scr(A1, JA1, IA1, n_z_el1, A2, JA2, IA2, n_z_el2, A3, JA3, IA3, &n_z_el3);

    time_e = tick() / (CLOCKS_PER_SEC / 1000000LLU);

    count_non_zero(matr3, n, m, &n_z_rows3, &n_z_el3);

    size_t size1 = sizeof(int)*n_z_el1*2 + sizeof(struct IA)*n_z_rows1;
    size_t size2 = sizeof(int)*n_z_el2*2 + sizeof(struct IA)*n_z_rows2;
    size_t size3 = sizeof(int)*n_z_el3*2 + sizeof(struct IA)*n_z_rows3;
